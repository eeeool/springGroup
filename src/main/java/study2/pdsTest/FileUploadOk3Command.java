...
public class FileUploadOk3Command implements CommonInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxSize = 1024 * 1024 * 30; // 서버에 저장시킬 최대 파일 용량을 10Mbyte 제한
		...
		while (fileNames.hasMoreElements()) {
			...
			oFileName += multipartRequest.getOriginalFileName("fName") + "/";
			fsName += multipartRequest.getFilesystemName("fName") + "/";
		}
		oFileName = oFileName.substring(0, oFileName.length()-1);
		fsName = oFileName.substring(0, fsName.length()-1);
				
		System.out.println("원본 파일명:" + oFileName);
		System.out.println("서버저장 파일명: " + fsName);
		System.out.println("서버파일 저장 경로명: " + realPath);
		
		// BackEnd 파일 체크
		if (fsName != null && !oFileName.equals("")) {
