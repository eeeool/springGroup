  ...
  <table class="table table-bordered m-0 p-0">
  	...
  	<tr>
  		...
  		<c:if test="${boardFlag == 'search'}">
  			<input type="button" value="돌아가기" onclick="location.href='BoardList.bo?search=${search}&searchString=${searchString}';" class="btn btn-warning" />
  		</c:if>
  		<c:if test="${boardFlag != 'search'}">
  		...
  		<td colspan="2" class="text-end">
	  		<c:if test="${sMid == vo.mid || sAdmin == 'adminOK'}">
	  			<c:if test="${sMid == vo.mid}">
		  			<input type="button" value="수정" onclick="location.href='BoardUpdate.bo?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}';" class="btn btn-danger" />
	  			</c:if>
	  			<input type="button" value="삭제" onclick="deleteCheck()" class="btn btn-success" />
	  		</c:if>
  		</td>
  	</tr>
  </table>
