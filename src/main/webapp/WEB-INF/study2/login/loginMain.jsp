    ...
    function loginList() {
    	$.ajax({
    		...
    		success: function(res) {
    			...
    			let tMid='', tNickName='닉네임: ', tName='성명: ', tAge='나이: ', tGender='성별: ', tAddress='주소: '
    			for (let j of js) {
    				tMid += j.mid + "/";
    			...
    			// 자바 객체로 변환된 자료를 테이블에 동적폼으로 출력시켜보자.
    			str = '<h4>회원리스트</h4>' 
    			str += '<table class="table table-hover text-center">';
    			str += '<tr class="table-secondary"><th>아이디</th><th>닉네임</th><th>성명</th><th>나이</th><th>성명</th><th>주소</th></tr>';
    			for (let j of js) {
    				str += '<tr>';
    				str += '<td>'+j.mid+'</td>';
    	...
    	$.ajax({
    		...
    		success:function(res) {
    			...
    			if (res.indexOf("/") != -1) {
    				...
    				str += '</tr>';
    				str += '<tr>';
    				str += '<td colspan="2" class="text-center"><input type="submit" value="정보수정" calss="btn btn-primary" /></td>'
    				str += '</tr>';
    				str += '</table>';
