...
insert into sungjuk values (default, '홍길동', 100, 90, 70);
insert into sungjuk values (default, '김말숙', 90, 90, 80);
insert into sungjuk values (default, '이기자', 70, 100, 80);
insert into sungjuk values (default, '오하늘', 90, 80, 70);
insert into sungjuk values (default, '김연아', 80, 90, 60);
insert into sungjuk values (default, '강감찬', 70, 80, 100);
insert into sungjuk values (default, '김 솔', 70, 80, 100);
...
select 123.456 as su, round(129.456, -1) as res;
...
select 123.456 as su, truncate(123.456, 3) as res;
select 123.456 as su, truncate(123.456, -1) as res;
...
-- 숫자 리스트중에서 가장 큰값(greatest),작은값(least)
...
-- 문자열 발췌: substring(데이터, 시작위치, 발췌개수): 시작위치 1번부터 처리
...
select datediff(now(), ipsail) from insa;
