...
/*
 	...
 	: 기본키 안의 필드를 줄수는 있으나, 독립필드로 여러개의 기본키는 허용하지 않는다.
...
/*
 	중복 불가능 키 (UNIQUE Key)
 	primary key를 대신해서 중복을 불허시키고자할때 사용하는 키 - 자동 인덱싱 된다.
 */
...
/*
 	외래키(foreign key): 하나의 테이블에서 다른 테이블의 정보를 찾기 위해서는 테이블 서로간의 관계설정을 하여야한다.
 		이때 주테이블의 키는 중복될수 없는키(primary key, unique key)가 되어야하고, 관계설정 맺은 테이블의 필드(키)를 외래키라고 한다.
*/
...
select * from hakkwa;
select * from hakkwa order by code;
drop table hakkwa;
...
create table student (
	...
	hakkwa_code INT, 	/* 학과 코드(hakkwa 테이블의 code필드와 관계설정) - 외래키 */
...
insert into student values (default, '홍길순', 104, 18);
insert into student values (default, '고인돌', 105, 22);
...
-- 일반조인 = Inner Join = 내부조인 = 교집함 --------------------------
select * from hakkwa INNER JOIN student where hakkwa.code = student.hakkwa_code;
select * from hakkwa INNER JOIN student where hakkwa.code = student.hakkwa_code order by name;
...
-- Left Join = Left Outer Join : 왼쪽 테이블이 기준이고, 오른쪽 테이블을 조합한다. (즉, 왼쪽테이블의 모든 필드가 출력되게한다.)
...
-- Right Join = Right Outer Join : 오른쪽 테이블이 기준이고, 왼쪽 테이블을 조합한다. (즉, 오른쪽테이블의 모든 필드가 출력되게한다.)
...
-- 합집합: 완전 내부조인(Full Outer Join) - Mysql에서는 지원하지 않는다.
-- SELECT * FROM hakkwa h FULL OUTER JOIN student s;
-- MySql에서는 union을 이용하여 합집합의 개념으로 사용한다. - Merge(병합)
-- 해당 테이블의 필드명이 같으면 병합 테이블로 만들어주지만, 필드명이 다르면 왼쪽 테이블 기준으로 처리한다. 
