...
INSERT INTO orders VALUES (12, 3, 18, 13000, '2024-12-19');
INSERT INTO orders VALUES (13, 3, 18, 13000, '2024-06-30');
INSERT INTO orders VALUES (14, 3, 18, 13000, '2024-11-21');
INSERT INTO orders VALUES (15, 3, 18, 13000, '2024-10-11');
...
-- 1번이상 구매한 책들의 정보?(구매 날짜순 내림차순) - 중복된 책들은 배제(1번 출력)
select distinct bookid, orderid, oderdate, concat(year(orderdate, -, month(orderdate), '-', day())) from orders order by orderdate desc;

select * from orders GROUP BY bookid;
select * from orders GROUP BY bookid order by orderdate desc;

-- 1번이상 구매한 사람들의 정보? (책 이름과 함께 출력)
select * from orders group by custid;
select o.* b.bookname from orders o, books b where o.bookid = b.bookid;

-- 1번이상 구마한 책들의 정보? (책 이름과 함께 출력)
select * from order o, books b where o.bookid = b.baookid;

-- 1번이상 구매한 책들의 정보? (주문번호,주문날짜,책이름, 정가,세일가)- 주문일자 내림차순
select o.orderid, o.orderdate, b.bookname, b.price ,o.saleprice from orders o, books b order books b where o.bookid = b.bookid;
...
select c.customer, count(o.custid), sum(o.saleprice),0), format(avg(o.saleprice), 0), avg(o.saleprice), b.bookname, o.saleprice, o.orderdate 
from orders o, books b, customers c
where o.bookid = b.bookid and o.custid = c.custid
group by c.customer;
 
select c.customer, count(o.custid), sum(c.customer) count(o.custid), sum(o.saleprice),0),, format(avg(o.saleprice), 0), avg(o.saleprice), b.bookname, o.saleprice, o.orderdate 
from orders o, books b, customers c
where o.bookid = b.bookid and o.custid = c.custid;
group by c.customer;),0), format(avg(o.saleprice), 0), avg(o.saleprice), b.bookname, o.saleprice, o.orderdate 
...
-- '대한미디어'나 '삼성당'에서 출판한 도서를 구매한 고객의 이름? (서브쿼리 사용)
select customer from customers 
where custid in (select custid from orders o 
where o.bookid in (select bookid from books 
where publisher = '대한미디어' or publisher = '삼성당'));

-- 1. '손흥민'이 구매한 도서의 이름, 정가, 구매가격, 할인받은금액(정가와 판매가격의 차)를 출력하시오.

-- 2. 고객별로 주문한 도서의 총 수량과 총 판매액을 구하시오. 이때 고객 아이디도 함께 출력하시오.
