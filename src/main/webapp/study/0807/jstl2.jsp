...
<body>
	...
	<div class="container">
		...
		<div>
			...
			6-3.str변수의 값 중에서 'o'문자가 들어가 있는 모든 위치?
			...
			6-4.str변수의 값중 'o'는 뒤에서부터 몇번째?(split/indexOf/substring/substringAfter/substringBefore()으로 검색)
				${fn:substringAfter(str, 'o')} / ${fn:substringBefore(str,'o')}<br/>
			7.문자열 분리(split() )? 'o'를 기준으로 분리<br/>
				<c:set var="positions" value="${fn:split(str, 'o')}" />
				<c:forEach var="p" items="${positions}" varStatus="st">
					${st.count}: ${p}<br/>
				</c:forEach>
				str 변수안의 'o'문자의 개수는? ${fn:length(positions)-1}<br/>
			8.치환(replace() ): 'o'문자를 'O'로 변경?
				<%=str.replace('o', 'O') %> / ${fn:replace(str, 'o', 'O')}<br/>
			9-1.문자열 포함 유무(contains() )? str변수에 'My' 포함 유무? ${fn:contains(str, 'My')}<br/>
			9-2.문자열 포함 유무(contains() )? str변수에 'My' 포함 유무? ${fn:contains(str, 'my')}<br/>
		</div>
	</div>
	<p>
		<br />
